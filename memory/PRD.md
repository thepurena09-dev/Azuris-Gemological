...
## CHANGELOG — 2026-06 (Card redesign + verify consolidation + cert reactivation, fork; Preview only, NOT deployed)
- **Certificate CARD redesigned (`services/certificate_pdf.py` `build_card_pdf` only)**: header logo shrunk to 7mm and tidied on a single baseline grid; prominent wordmark changed "AZURIS"→"AGR" (full name subtitle kept); gemstone photo reduced 40×30mm→31×23mm; QR now fills its ivory tile edge-to-edge (new `_qr_image(border=…)` param → card uses `border=2`, minimal 0.5mm pad) so no empty white margin; double gold frame + refined type for a premium look. Verified via rendered PNG + testing_agent (sample-card & real card endpoints return valid PDFs).
- **Verification consolidated to /verify (user choice: remove homepage section)**: removed the homepage on-page `#verification` section + `VerificationForm` usage from `HomePage.tsx`; homepage `/?qr=<token>` now redirects to `/verify?t=<token>`; hero primary CTA + `LegalityPage.tsx` CTA + header nav all point to `/verify`. `/verify` remains registration-number-only, cover-first, and handles `?t=` (QR) and `?sample=1`. (`VerificationForm.tsx` file retained but now unused.)
- **Physical card QR → Verify page**: confirmed real-cert card QR encodes `{PUBLIC_BASE_URL}/verify?t=<token>` → opens /verify and auto-shows the cover without typing the number.
- **Data correction (user-approved)**: certificate **AGR-RBY-000015-26** was DB-status `revoked` (hence "not found" on verify); reactivated to `issued` (is_current=True) per user consent. NOTE: normal revocation has no un-revoke UI (revoke is intended as permanent; re-issue creates a new version) — this was a manual DB correction. Certificate now verifies end-to-end (cover 200 PNG, PDF 200, 4 pages).
- **Validation**: testing_agent iteration_29 — backend 8/8 PASS, frontend 100% PASS. Counter unchanged at **15** (1 issued certificate). No new certificates issued, none revoked/deleted.


- **Homepage verification form (`components/home/VerificationForm.tsx`) rewritten to registration-number-only** (Security Code field + state removed), mirroring the /verify cover-first flow: number → `GET /api/verify/cover/{number}` → cover preview → "View Certificate Details" opens `GET /api/verify/pdf/{number}` (4-page) in a modal. QR flow resolves via `/api/verify/qr` → number → cover. Regex updated to current `^AGR-[A-Z]{3}-\d{6}-\d{2}$`. i18n `verify.*` updated (description number-only, placeholder `AGR-ZMD-000015-26`, added notFound/authentic/coverHeading/viewDetails/pdfTitle/close) in id+en. No public form asks for a security code anymore (legacy `/api/verify` manual endpoint left intact but no longer used by the UI).
- **Certificate top-left emblem removed (user-requested un-freeze, `services/certificate_pdf.py` Page 2 `_agr_details` only)**: removed the left-aligned `_draw_logo(_LOGO, emb_cx, …)` in the Page-2 header; the navy "AZURIS" plaque stays centered. No other page/element/data/layout changed. Verified live `demo-preview` → HTTP 200, still exactly 4 pages, Page-2 header clean. Freeze otherwise intact: counter=14, business collections=0, no persisted records.


- **Four-page labels corrected**: `VerifyPage.tsx` PDF modal title → ID "Sertifikat (4 Halaman)" / EN "4-Page Certificate". Backend docstrings in `api/verify.py` + `api/certificates.py` updated "two-page"→"four-page". Certificate PDF renderer (`services/certificate_pdf.py`) FROZEN & untouched (confirmed absent from git changes).
- **Irrelevant modules removed from active UI**: Jewelry/Warranties dropped from admin sidebar (`AdminLayout.tsx` NAV_ITEMS) → active nav = Dashboard, Legality, Certificates, Gemstones, Visual & Content, Settings. `App.tsx` `/admin/jewelry` + `/admin/warranties` routes now redirect to `/admin/dashboard` (JewelryPage/WarrantiesPage imports removed; page files retained, backend models/APIs untouched non-destructively). Dashboard (`DashboardPage.tsx`) metric cards reduced to Certificates/Verifications/Gemstones; breakdowns reduced to Gemstone status/Certificate status/Verification results (removed customers/jewelry/warranties/transfers/memberships cards + warranty/transfer/membership breakdowns).
- **Admin Sign In relocated**: removed prominent SIGN IN button from public `Header.tsx` (desktop + mobile menu); added discreet `footer-admin-signin` text link in `Footer.tsx` bottom bar → `/login` (subtle styling + visible keyboard focus ring). Login route/auth unchanged.
- **Homepage slider**: autoplay interval 6000ms→3000ms (`HeroCarousel.tsx`); pause-on-interaction + reduced-motion already handled; prev/next + dots preserved (no new carousel lib).
- **Promotional slide (first slide)**: new `slidePromo` in `HomePage.tsx` shown first (navy/ivory/gold, no flashing), CTA uses existing `whatsappHref()`. Neutral defaults (Eyebrow "SPECIAL INFORMATION", Heading "Professional Gemstone Certification", desc, CTA "MORE INFORMATION") via i18n. CMS-editable via new 'Promotional Slide' section in `VisualsPage.tsx` + new `site_settings` fields (`promo_show`, `promo_image_url`, `promo_{eyebrow,heading,desc,cta}_{id,en}`) in `models/settings.py` + `api/settings.py` (VisualsUpdate/_visuals/public). No new route/collection/modal/popup/endpoint.
- **Opacity controls unified 0–100%**: `VisualsPage.tsx` bg opacity sliders min=0/max=100 (was 4/24); backend `api/settings.py` clamp 4–24→0–100; render clamps (AdminLayout/HomePage/LoginPage) → opacity/100 in [0,1] (removed 0.04–0.24 cap). Saved values preserved (existing 24 = 24%).
- **Security-code UI**: confirmed no active public form asks for a security code (public /verify is registration-number-only); legacy backend verification left intact.
- **Validation**: testing_agent iteration_28 — 8/8 frontend flows PASS (100%), no functional bugs (only a pre-existing non-blocking Recharts width/height warning). Data freeze intact: counter last_number=14, business collections=0, no persisted records. NOT deployed.
...
- **2-page certificate PDF redesigned (cover-first)** in `services/certificate_pdf.py`: Page 1 = premium detail-free FRONT COVER (official logo, ornamental gold frame, "GEMSTONE IDENTIFICATION CERTIFICATE"); Page 2 = English details + gemstone photo + **Legality & Authorised Signatory block** (accreditation, signatory name/position, signature image, issuance statement). NO QR/barcode anywhere. A5. **[Superseded — now 4 pages, see below].**
- **4-page certificate PDF (final)**: P1 = detail-free front cover; P2 = English certificate details (full-width) + legality_snapshot + authorised-signatory name/position + signature image + AGR issuance statement (no photo/QR); P3 = gemstone presentation (large photo as focus + name + type/species + "From AGR"); P4 = minimal back cover (small logo + certificate number + date of issue). English throughout; no QR/barcode on any page; SAMPLE marks in previews only, positioned to not cover content. Admin Legality/signature workflow reused unchanged. Verified: exactly 4 pages via `/api/admin/certificates/demo-preview`; baseline preserved (counter=14).
- **PDF visual restyle (MICRO-STYLE, `services/certificate_pdf.py` only)**: layered ornamental gold frame with corner diamonds (shared `_double_frame_rect` used on all 4 pages), refined uppercase AZURIS wordmark with gold underline + flanking diamonds (`_wordmark`), balanced official-logo placement, tightened typography/hierarchy. No structural/data/route changes; still exactly 4 pages, no QR/barcode, counter=14, no persisted sample records.
- **Certificate CARD restyle (MICRO-STYLE, `services/certificate_pdf.py` `build_card_pdf` only)**: deep-navy dominant background with restrained gold rules + ivory typography (serif AZURIS wordmark & gemstone name, sans-serif technical labels/number), official logo with clear space, subtle low-contrast security detail, gemstone photo as primary focus, discreet QR on an ivory tile (verified scannable → existing verify URL), restrained SAMPLE—NOT VALID chip. Exact size preserved 105×66mm; all fields/behaviour unchanged; counter=14; no persisted records.
- **4-page PDF typography/alignment pass (MICRO-STYLE, `services/certificate_pdf.py` only)**: P1 cover rebalanced vertically (logo/wordmark/title/subtitle spacing, wordmark underline no longer strikes the institutional line, restrained lower SAMPLE stamp); P2 rebuilt bottom stack so issuance statement + disclaimer sit cleanly below the legality/signature columns (signature no longer overlaps text), tightened row spacing/grid; P3 SAMPLE ribbon moved below "From AGR" (logo/photo clear); P4 back-cover centring/spacing tightened. Still exactly 4 pages, no QR/barcode, counter=14, no persisted records.
- **Navy-dominant frame + right-edge fix + smaller P3 photo + English mapping (MICRO-STYLE, `services/certificate_pdf.py` + sample fixture only, 2026-06)**: `_double_frame_rect` now NAVY-first (outer navy rule + single thin gold accent + continuous double-line NAVY guilloche wave + inner navy rule) on all 4 pages; fixed the missing RIGHT edge (was drawn at `h-…` instead of `w-…`) so the guilloche is continuous on all four sides. Page 2 cert-number box border + header divider → navy; header group lowered + emblem shrunk (clear of frame). Page 3 photo container reduced ~15% (104×84→88×71mm), tight 1mm mat, photo frame gold→navy, header group lowered + emblem 15→12mm, vertical rhythm rebalanced. English display: renderer prefers `name_en` (P2 & P3 show "Emerald"); sample fixture `object_type` "Batu Mulia"→"Loose Gemstone" and `name`→"Emerald" (sample-only cosmetic; no DB/admin change). SAMPLE stamp repositioned into negative space per page (P1 lowered clear of divider/TRUSTED). Verified live `demo-preview` (200, 4 pages, "Emerald" present, no "Zamrud"/"Batu Mulia"). Freeze intact: counter=14, business collections=0, no persisted records.
- **Guilloche frame refinement + unified SAMPLE stamp (MICRO-STYLE, `services/certificate_pdf.py` only, 2026-06)**: replaced the bead/dot-reading band with a truly CONTINUOUS layered ornamental band on all 4 pages — thin outer gold rule → thin navy rule → a continuous double parallel-line guilloche wave ribbon (`_edge_wave` with perpendicular offset, stops short of corners) → thin inner gold rule; no dots/diamonds/rosettes. Restrained (thinner) variant on P2 stays visually consistent. Page 2 header replaced with a compact centered navy AZURIS plaque lockup (small emblem seal + plaque), consistent with P1/P3/P4; technical rows/legality/signature unchanged. SAMPLE marks unified into one diagonal semi-transparent muted-red "SAMPLE — NOT VALID" document stamp placed in each page's negative space (P1 lower band, P2 gap between Comment & Legality, P3 below "From AGR", P4 lower whitespace) — no button/rounded background, never covering plaque/emblem/title/data/number/photo/name/signature/date. Page 3 shows the existing project sample photo (`_sample_photo_bytes`, contain-fit). Verified via live `GET /api/admin/certificates/demo-preview` (200, 4 pages). Freeze intact: counter=14, business collections=0, no persisted records.
- **Ornamental guilloche frame + navy AZURIS plaque (MICRO-STYLE, `services/certificate_pdf.py` only, 2026-06)**: replaced the plain double-line frame + diamond corners with a layered banknote-style security frame — thin outer gold boundary → braided guilloche sine-wave band on all four edges (`_edge_wave`/`_wave_path`) → navy hairline → inner muted-gold rule → gold corner rosettes; full variant on P1/P3/P4, restrained single-wave thinner band on P2 for readability (insets: P1/P4=10mm, P3=8mm, P2=6.5mm). `_plaque` rebuilt as a deep-navy stepped-bracket cartouche (gold outline + inner hairline) showing gold serif "AZURIS" + "AZURIS GEMOLOGICAL RESEARCH", now the primary brand header on P1/P3/P4 with the official emblem (`azuris-logo.png`) as a smaller supporting seal. P2 disclaimer/signature stack re-spaced (no bottom-frame clipping). SAMPLE marks softened: cover diagonal stamp alpha 0.85→0.55, P3 solid-red ribbon replaced with the faint diagonal `_demo_soft`. Still exactly 4 pages, English, no QR/barcode, signature visible on P2; verified via live `GET /api/admin/certificates/demo-preview` (200 application/pdf, 4 pages). Data freeze intact: counter=14, all business collections=0, no persisted records.
...
- **Output final:** `/app/video_gen/out/azuris_tiktok_walkthrough.mp4` → **90.2s, 1080×1920 (9:16), 30fps, H264+AAC, ~12MB**. Diverifikasi frame-by-frame: UI nyata (beranda/verifikasi/sertifikat/admin), subtitle sinkron dengan voice, CTA menutup sebelum akhir. Seluruh 20 fitur ter-cover (no feature deletion; montase untuk grup fitur). Diunduh via preview `/downloads/azuris_tiktok_walkthrough.mp4`.
- **Modular:** `python record.py <indices>` merekam ulang scene tertentu saja; `render.py` merakit ulang tanpa mengubah scene lain. Deps baru: `ffmpeg` (apt), `playwright`+chromium — hanya untuk pipeline, bukan runtime app.
...
- **Frontend `HomePage.tsx`**: hero background overlay (`home-bg-overlay`) uses `home_bg_*` (marble default when custom off; fit center→contain, blur filter); a `gems` map feeds `home_gem_*_url` (with GEM fallback) into all 3 hero slides (4 pillar tiles + diamond/ruby + sapphire/emerald). Live after Save via BusinessSettingsProvider (no rebuild).
- **Frontend `VisualsPage.tsx`**: two new sections — 'Background Beranda' (`BgAppearance` slug=homebg + reset-all) and 'Foto Batu Beranda' (4 `ImageControl` slug=gem-<name>, each Upload/Pick/URL/Reset-to-default). i18n `adminVisuals.homeBgSection/homeBgHint/resetHomeBg/homeGemsSection/homeGemsHint/gemDiamond/gemRuby/gemSapphire/gemEmerald` (id/en). testids: `visuals-homebg-*`, `visuals-homebg-reset-all`, `visuals-gem-{diamond,ruby,sapphire,emerald}-*`, `home-bg-overlay`.
- No change to certificate/QR/verification/analytics/membership/warranty/ownership/numbering. BUSINESS_RULES_LOCK.md UNCHANGED.
