...
- **Certificate CARD restyle (MICRO-STYLE, `services/certificate_pdf.py` `build_card_pdf` only)**: deep-navy dominant background with restrained gold rules + ivory typography (serif AZURIS wordmark & gemstone name, sans-serif technical labels/number), official logo with clear space, subtle low-contrast security detail (gold pattern + faint gem emblem), gemstone photo as primary focus, discreet QR on an ivory tile (verified scannable → existing verify URL), restrained SAMPLE—NOT VALID chip not covering key content. Exact size preserved 105×66mm; all fields/behaviour unchanged; counter=14; no persisted records.
